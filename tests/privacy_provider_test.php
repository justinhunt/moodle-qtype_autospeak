...
use qtype_autospeak\privacy\provider;
...
global $CFG;
require_once($CFG->dirroot . '/question/type/autospeak/classes/privacy/provider.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_autospeak_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {
    // Include the privacy helper which has assertions on it.

    public function test_get_metadata() {
        $collection = new \core_privacy\local\metadata\collection('qtype_autospeak');
        $actual = \qtype_autospeak\privacy\provider::get_metadata($collection);
    ...
    public function test_export_user_preferences($name, $value, $expected) {
        $this->resetAfterTest();
        $user = $this->getDataGenerator()->create_user();
        set_user_preference("qtype_autospeak_$name", $value, $user);
        provider::export_user_preferences($user->id);
        $writer = writer::with_context(\context_system::instance());
        $this->assertTrue($writer->has_any_data());
        $preferences = $writer->get_user_preferences('qtype_autospeak');
        foreach ($preferences as $key => $pref) {
            $preference = get_user_preferences("qtype_autospeak_{$key}", null, $user->id);
            if ($preference === null) {
                continue;
            }
            $desc = get_string("privacy:preference:{$key}", 'qtype_autospeak');
